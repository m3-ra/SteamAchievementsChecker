...
class FetchAchievementsOnlineUseCaseTest {
    ...
    fun `SHOULD fetch online then add achievements percentages WHEN getting games info`() =
        runTest {
            ...
            } returns AchievementsResult.HasAchievements(50)
            coEvery {
                achievementsRepository.getAchievementsPercentageByGame(2)
            } returns AchievementsResult.HasAchievements(100)
            coEvery {
    ...
    fun `SHOULD save game info to db WHEN data has been fetched`() =
        runTest {
            ...
            // Assert
            val expected = listOf(GameInfo(1, "Game xyz", AchievementsResult.HasAchievements(50)))
    ...
    fun `SHOULD handle mixed achievements WHEN some games have no achievements`() =
        runTest {
            ...
            } returns AchievementsResult.HasAchievements(0)
            ...
            // Assert
            val expected = listOf(
                GameInfo(1, "Game with achievements", AchievementsResult.HasAchievements(50)),
                GameInfo(2, "Game without achievements", AchievementsResult.NoAchievements),
                GameInfo(3, "Game fully completed", AchievementsResult.HasAchievements(100)),
                GameInfo(4, "Another game without achievements", AchievementsResult.NoAchievements),
                GameInfo(5, "Game partially completed", AchievementsResult.HasAchievements(0))
            )
            assertEquals(expected, result)
        }
}
